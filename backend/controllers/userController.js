...
const userController = {
  createBulkNewUsers: async (req, res) => {
    try {
      const users = req.body; // Anda dapat menerima array pengguna dari req.body

      // Melakukan iterasi melalui setiap pengguna
      for (let user of users) {
        // Menghasilkan salt dan menghash password
        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(user.password, salt);

        // Mengganti password asli dengan password yang di-hash
        user.password = hashedPassword;
      }

      // Memasukkan pengguna yang telah di-hash ke dalam database
